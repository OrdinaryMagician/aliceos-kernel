...
void OHSHI( char *mesg, regs_t regs )
{
	...
	mode_3h.fbprintf("%[7%{17,12SS  %#08x",regs.ss);
	mode_3h.fbprintf("%[7%{32,12CS  %#08x",regs.cs);
	mode_3h.fbprintf("%[7%{47,12DS  %#08x",regs.ds);
	mode_3h.fbprintf("%[7%{2,14EFLAGS %#08x [",regs.eflags);
	for ( i=0; i<32; i++ )
